...
<template>
  <div>
    ...
    <!-- 分页签：这些检查项一屏放不下，按主题分开；「待处理」是异常的唯一出口，默认打开。
         当前页签写进地址栏（?tab=…），报告或横幅指过来的链接能直接落在对应页签上。 -->
    <el-tabs v-model="tab" class="fnwg-maint-tabs">
      <el-tab-pane name="issues">
        <template #label>
          <span class="fnwg-maint-tab">
            待处理
            <el-tag
              v-if="issues.length"
              size="small"
      ...
      <!-- 系统网络：NAS 自身上网路线 + 飞牛桌面入口（都属于「这台机器通不通」） -->
      <el-tab-pane label="系统网络" name="network">
      ...
      <!-- 内网访问：逐层诊断 + 访问控制配置检查 -->
      <el-tab-pane label="内网访问" name="lan">
        ...
        <div class="fnwg-card">
          <div class="fnwg-card-head">
            ...
            <el-tag v-if="!lanOn && net" size="small" type="info" effect="plain">未启用</el-tag>
            <el-tag v-else-if="natFailed.length" size="small" type="danger" effect="plain">
              {{ natFailed.length }} 项未通过
            </el-tag>
            <el-tag v-else-if="net" size="small" type="success" effect="plain">全部通过</el-tag>
          </div>

          <div v-if="!lanOn && net" class="fnwg-hint">
            没有连接打开「允许设备访问家里内网」，这一层不需要生效 —— 下面的检查项按「未启用」显示。
          </div>
          ...
          <div v-for="c in net?.nat?.checks || []" :key="c.key" class="fnwg-nat-check">
            <el-tag
              size="small"
              :type="c.ok ? 'success' : lanOn ? 'danger' : 'info'"
              effect="plain"
            >
              {{ c.ok ? '通过' : lanOn ? '未通过' : '未启用' }}
            </el-tag>
            <div class="fnwg-nat-check-body">
      ...
      <!-- 巡检报告：计划 + 最近一次报告 + 历史 -->
      <el-tab-pane label="巡检报告" name="inspect">
 ...
 * 当前页签。
 *
 * 写进地址栏（?tab=…）：报告里「去「待处理」修复」这类跳转能直接落在对应页签上，
 * 刷新也不会跳回第一页。非法值一律回落到「待处理」—— 那是所有异常的唯一出口。
 */
const tabs = ['issues', 'network', 'lan', 'inspect', 'runtime'] as const
type MaintTab = (typeof tabs)[number]
const tab = ref<MaintTab>(
  tabs.includes(route.query.tab as MaintTab) ? (route.query.tab as MaintTab) : 'issues',
)
watch(tab, (v) => {
 ...
 * 内网访问是否真的被某条连接启用了。
 *
 * 没启用时后端仍会给出「尚未下发：没有打开内网访问开关的连接」这类检查项；
 * 把它们标成「未通过」，就会与上方「待处理事项：全部通过」自相矛盾 ——
 * 同一件事两种说法，而用户什么都没做错。判据取结构字段（规则是否生效、放行了哪些源网段），
 * 不认文案：文案会改，字段不会骗人。
 */
const lanOn = computed(() => !!(net.value?.nat?.active || (net.value?.nat?.sources || []).length))
...
<style scoped>
/* 页签：卡片之间本就靠 fnwg-card 分隔，这里只给它一点上边距 */
.fnwg-maint-tabs {
  margin-top: 4px;
}
