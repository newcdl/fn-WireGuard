...
**1.0 正式版**（`dev` 分支，即将发布）—— 0.9.x 的全部内容收敛为一个正式版：
...
- **登录页重新设计**：桌面双栏（品牌栏 + 表单）、窄屏单栏，明暗主题各自适配
- 缺陷修复：未配置对外地址时「需重新扫码」误报、拓扑卡片被画布裁掉与缩略图遮挡、二次验证二维码首次空白、应急安全码状态误报「未设置」、说明文案里的 Markdown 星号、操作列「更多」下拉偏高

**规划中**：1.0 之后的版本见 [docs/ROADMAP.md](docs/ROADMAP.md)。
