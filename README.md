...
我的连接   创建/编辑/启停连接、地址端口 MTU DNS、内网访问与设备隔离开关、场景预设、导入导出
我的设备   设备管理、二维码、上网方式与通行范围、使用期限、配额、配置过期提示
运行记录   操作审计与系统日志
系统维护   一键体检、异常清单、网络修复、内网访问诊断、访问控制配置检查、残留网卡清理、重新应用
系统设置   接入设置、事件通知、账号管理、登录方式、备份还原、配置快照、关于
```
...
- **飞牛账号免密登录**：从飞牛桌面打开本应用即直接进入，不用再输一遍账号密码——身份由飞牛统一网关校验后注入，飞牛的管理员对应本应用管理员，普通用户为只读。也可以在「系统设置 → 登录方式」里关掉端口登录、只留飞牛账号。
...
- **删除全部数据**：数据库、主密钥、安全码与历史备份一并清空，卸载后无法找回——建议先在「系统设置 → 备份」导出备份再卸载。
...
### Q10. 界面进不去了（忘记密码、换了手机、或关掉了端口登录）

按「最省事 → 最彻底」的顺序试：

1. **从飞牛桌面打开本应用**：飞牛的账号体系校验通过后即可免密进入；飞牛的管理员在本应用里就是管理员，进去后可在「系统设置 → 账号管理」里重置密码。
2. **用安全码应急登录**：登录页点「密码和验证码都用不了？用安全码应急登录」。它一直可用（不受「登录方式」开关影响），只能使用一次，用完即作废并下发新的一码。
3. 以上都不行时，在飞牛 **应用中心 → fn-WireGuard → 停用再启用**，或执行
   `sudo /usr/local/bin/fnwg-cli status` 确认服务状态；账号数据在停用时保留，卸载时由你在卸载向导里选择保留或删除。

> 提醒：把「登录方式」切成「只用飞牛账号」之前，系统会要求你**先成功用飞牛账号进入过一次**。少了这道确认，万一手一抖关掉端口登录、而网关又恰好不通，就真的进不去了——这是刻意的设计，不是多余的步骤。
...
规划中：0.8.0 内网体验、0.9.0 兼容性与账号安全、1.0.0 正式版。
