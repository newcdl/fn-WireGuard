...
- **0.9.1** 修缺陷：部分更新（例如只改「允许设备访问家里内网」）不再把 `enabled` / `autostart` 这类没传的开关一并关掉
- **0.9.3** 计划备份：按日程把完整备份写到用户授权的共享文件夹（在飞牛的「应用设置 → 授权目录」里授权后从列表中选择），保留最近若干份；保存时探测目标目录可写，错过的时间点开机后补跑，失败推送通知，可从副本直接下载或还原
- **0.9.2** 流量记账、每月额度与流量报表：
  - 记账：NAS 每分钟把每台设备的累计流量折算成增量、按小时落库；保留期默认 90 天且可调，报表页显示已存条数与最早一条
  - 报表：运行记录 → 流量报表，按设备/按天查看用量与趋势（近 7 / 30 / 90 天），一键导出 CSV（带 BOM，Excel 双击不乱码）
  - 每月额度：按自然月计发送量，用满自动停用并推送通知，月初自动恢复；自动停用的原因写进审计与设备列表
  - 我的设备新增「本月用量」列，配了额度的设备以进度条对照

**规划中** —— **1.0.0 正式版**：多 NAS 互联模板、计划备份到外部存储、配置漂移巡检报告。
