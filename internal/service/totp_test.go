...
func TestTOTPEnableAndLogin(t *testing.T) {
	...
	step, err := svc.Login(ctx, "admin", "admin12345", "ua", "127.0.0.1")
	if err != nil {
	...
	// 错误动态口令进不来
	if _, err := svc.CompleteTOTPLogin(ctx, step.Challenge, "000000", "ua", "127.0.0.1"); err == nil {
	...
	done, err := svc.CompleteTOTPLogin(ctx, step.Challenge, code, "ua", "127.0.0.1")
	if err != nil {
	...
	if _, err := svc.CompleteTOTPLogin(ctx, step.Challenge, code, "ua", "127.0.0.1"); err == nil {
		t.Fatal("挑战用过一次后应当失效")
	}

	// 恢复码：可用，且用完即废
	step2, err := svc.Login(ctx, "admin", "admin12345", "ua", "127.0.0.1")
	if err != nil {
		t.Fatal(err)
	}
	rec := codes[0]
	done2, err := svc.CompleteTOTPLogin(ctx, step2.Challenge, rec, "ua", "127.0.0.1")
	if err != nil {
	...
	step3, err := svc.Login(ctx, "admin", "admin12345", "ua", "127.0.0.1")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.CompleteTOTPLogin(ctx, step3.Challenge, rec, "ua", "127.0.0.1"); err == nil {
		t.Fatal("同一枚恢复码不应能重复使用")
	}
	// 恢复码允许带连字符/小写输入（用户是照着手抄的）
	step4, err := svc.Login(ctx, "admin", "admin12345", "ua", "127.0.0.1")
	if err != nil {
		t.Fatal(err)
	}
	loose := strings.ToLower(strings.ReplaceAll(codes[1], "-", " "))
	if _, err := svc.CompleteTOTPLogin(ctx, step4.Challenge, loose, "ua", "127.0.0.1"); err != nil {
...
func TestTOTPChallengeAttemptLimit(t *testing.T) {
	...
	if _, err := svc.CompleteTOTPLogin(ctx, step2.Challenge, code2, "ua", "127.0.0.1"); err != nil {
